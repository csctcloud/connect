...
!!! tip "UWE lab computers"
    We need to instruct the Remote Development extensions to store the SSH configuration file on OneDrive.
...
Visual Studio Code can syncronise all the settings and extensions you setup using *Settings Sync*. You don't need to use this if you're using your own personal computer, but you will need to set this up if using UWE lab computers.
...
!!! tip "UWE lab computers"
    You will need to turn on Settings Sync every time you use Visual Studio Code on a new lab computer, so that it's setup to use the SSH configuration file stored on OneDrive.
...
The *Explorer* tab will let you open folders within your home directory on the server which you can create new files and work in, and you can launch a new terminal window within Visual Studio Code using the *Terminal* menu at the top.
